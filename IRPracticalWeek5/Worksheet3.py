...
def print_skip_list(list):
    if list.length == 0:
        print('None Elements in List')
    else:
        node = list.start
...
if __name__ == '__main__':
    ...
    list2 = SkipList([2, 3, 46, 70, 7222, 100001])
    # list_ans = intersect(list1, list2)
    # print_skip_list(list_ans)
    intersect_skip(list1, list2)
    ...
    time_taken = timeit.timeit('intersect_skip( list1, list2 )', number=10, globals=globals())
    print('Execution took {:.4f} seconds'.format(time_taken))
